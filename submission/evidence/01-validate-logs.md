...
Output thật, chạy 2026-08-11 trên `data/logs.jsonl` sau khi hoàn tất baseline (64 request)
...
```json
{"service": "api", "payload": {"message_preview": "Email [REDACTED_EMAIL], SDT [REDACTED_PHONE_VN], CCCD [REDACTED_CCCD], the [REDA..."}, "event": "request_received", "session_id": "demo-s3", "env": "dev", "feature": "qa", "user_id_hash": "cebf29_2c038f", "correlation_id": "req-9f2448f9", "model": "claude-sonnet-4-5", "level": "info", "ts": "2026-08-11T10:11:16.182865Z"}
{"service": "api", "latency_ms": 150, "tokens_in": 52, "tokens_out": 170, "cost_usd": 0.002706, "quality_score": 0.8, "payload": {"answer_preview": "Starter answer. Teams should improve this output logic and add better quality ch..."}, "event": "response_sent", "session_id": "demo-s3", "env": "dev", "feature": "qa", "user_id_hash": "cebf29_2c038f", "correlation_id": "req-9f2448f9", "model": "claude-sonnet-4-5", "level": "info", "ts": "2026-08-11T10:11:16.334356Z"}
```

Lệnh tái hiện:

```bash
python -c "import json;[print(l.strip()) for l in open('data/logs.jsonl',encoding='utf-8') if 'req-9f2448f9' in l]"
```
...
|---|---|
| Correlation ID xuyên suốt | `req-9f2448f9` xuất hiện ở **cả** `request_received` và `response_sent` |
...
| `user_id` không lộ | `student-01` → `cebf29_2c038f`, không ghi user_id thô |
...
`cebf29_2c038f` là sha256 rút gọn, chèn `_` vào giữa. Lý do: hash 12 hex liền mạch có
